...
class ZionWebSupabaseUser extends BaseAuthUser {
  ZionWebSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> zionWebSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ZionWebSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
